...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{var} \NormalTok{postData = }\OtherTok{querystring}\NormalTok{.}\FunctionTok{stringify}\NormalTok{(\{}
  \StringTok{'msg'} \NormalTok{: }\StringTok{'Hello World!'}
\NormalTok{\});}
...
\KeywordTok{var} \NormalTok{options = \{}
  ...
  \DataTypeTok{method}\NormalTok{: }\StringTok{'POST'}\NormalTok{,}
  \DataTypeTok{headers}\NormalTok{: \{}
    \StringTok{'Content-Type'}\NormalTok{: }\StringTok{'application/x-www-form-urlencoded'}\NormalTok{,}
    \StringTok{'Content-Length'}\NormalTok{: }\OtherTok{postData}\NormalTok{.}\FunctionTok{length}
  \NormalTok{\}}
\NormalTok{\};}
...
\KeywordTok{var} \NormalTok{req = }\OtherTok{http}\NormalTok{.}\FunctionTok{request}\NormalTok{(options, }\KeywordTok{function}\NormalTok{(res) \{}
  ...
  \KeywordTok{var} \NormalTok{data = }\StringTok{''}\NormalTok{;}
  \OtherTok{res}\NormalTok{.}\FunctionTok{on}\NormalTok{(}\StringTok{'data'}\NormalTok{, }\KeywordTok{function} \NormalTok{(chunk) \{}
    \OtherTok{console}\NormalTok{.}\FunctionTok{log}\NormalTok{(}\StringTok{'PARTIAL BODY: '} \NormalTok{+ chunk);}
    \NormalTok{data += chunk;}
  \NormalTok{\});}
  \OtherTok{res}\NormalTok{.}\FunctionTok{on}\NormalTok{(}\StringTok{'end'}\NormalTok{, }\KeywordTok{function}\NormalTok{() \{}
    \OtherTok{console}\NormalTok{.}\FunctionTok{log}\NormalTok{(}\StringTok{'COMPLETE BODY: '} \NormalTok{+ data);  }
  \NormalTok{\});}
\NormalTok{\});}
...
\OtherTok{req}\NormalTok{.}\FunctionTok{write}\NormalTok{(postData);}
...
\OtherTok{http}\NormalTok{.}\FunctionTok{get}\NormalTok{(}\StringTok{"http://www.google.com/index.html"}\NormalTok{, }\KeywordTok{function}\NormalTok{(res) \{}
  ...
  \CommentTok{// consume response body}
  \OtherTok{res}\NormalTok{.}\FunctionTok{resume}\NormalTok{();}
...
\subsubsection{\texorpdfstring{Event:
`upgrade'}{Event: upgrade}}\label{event-upgrade-1}
