...
Stream} interface. \texttt{http.IncomingMessage} is an
\href{events.html\#events\_class\_events\_eventemitter}{EventEmitter}
with the following events:
...
string if an encoding has been set with \texttt{message.setEncoding()},
...
\texttt{IncomingMessage} emits a \texttt{'data'} event.

\subsubsection{Event: `end'}

\texttt{function () \{ \}}

Emitted exactly once for each response. After that, no more
...
\subsubsection{message.setEncoding({[}encoding{]})}

Set the encoding for data emitted by the \texttt{'data'} event. See
\href{stream.html\#stream\_stream\_setencoding\_encoding}{stream.setEncoding()}
for more information.

Should be set before any \texttt{'data'} events have been emitted.

\subsubsection{message.pause()}

Pauses request/response from emitting events. Useful to throttle back a
download.

\subsubsection{message.resume()}
