...
\texttt{Buffer} can also be used with Typed Array Views and DataViews.

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{var} \NormalTok{buff = }\KeywordTok{new} \NormalTok{Buffer(}\DecValTok{4}\NormalTok{);}
\KeywordTok{var} \NormalTok{ui16 = }\KeywordTok{new} \NormalTok{Uint16Array(buff);}
\KeywordTok{var} \NormalTok{view = }\KeywordTok{new} \NormalTok{DataView(buff);}

\NormalTok{ui16[}\DecValTok{0}\NormalTok{] = }\DecValTok{1}\NormalTok{;}
\NormalTok{ui16[}\DecValTok{1}\NormalTok{] = }\DecValTok{2}\NormalTok{;}
\KeywordTok{console}\NormalTok{.}\FunctionTok{log}\NormalTok{(buff);}

\KeywordTok{view}\NormalTok{.}\FunctionTok{setInt16}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{);       }\CommentTok{// set big-endian int16 at byte offset 0}
\KeywordTok{view}\NormalTok{.}\FunctionTok{setInt16}\NormalTok{(}\DecValTok{2}\NormalTok{, }\DecValTok{2}\NormalTok{, }\KeywordTok{true}\NormalTok{); }\CommentTok{// set little-endian int16 at byte offset 2}
\KeywordTok{console}\NormalTok{.}\FunctionTok{log}\NormalTok{(buff);}

\CommentTok{// <Buffer 01 00 02 00>}
\CommentTok{// <Buffer 00 01 02 00>}
\end{Highlighting}
\end{Shaded}
