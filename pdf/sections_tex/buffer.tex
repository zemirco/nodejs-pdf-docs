...
\KeywordTok{var} \NormalTok{copy = }\KeywordTok{JSON}\NormalTok{.}\FunctionTok{parse}\NormalTok{(json, }\KeywordTok{function}\NormalTok{(key, value) \{}
    \KeywordTok{return} \NormalTok{value && }\KeywordTok{value}\NormalTok{.}\FunctionTok{type} \NormalTok{=== }\CharTok{'Buffer'}
      \NormalTok{? }\KeywordTok{new} \NormalTok{Buffer(}\KeywordTok{value}\NormalTok{.}\FunctionTok{data}\NormalTok{)}
      \NormalTok{: value;}
  \NormalTok{\});}
...
\KeywordTok{socket}\NormalTok{.}\FunctionTok{on}\NormalTok{(}\CharTok{'readable'}\NormalTok{, }\KeywordTok{function}\NormalTok{() \{}
  \KeywordTok{var} \NormalTok{data = }\KeywordTok{socket}\NormalTok{.}\FunctionTok{read}\NormalTok{();}
  \CommentTok{// allocate for retained data}
  \KeywordTok{var} \NormalTok{sb = }\KeywordTok{new} \NormalTok{SlowBuffer(}\DecValTok{10}\NormalTok{);}
  \CommentTok{// copy the data into the new allocation}
  \KeywordTok{data}\NormalTok{.}\FunctionTok{copy}\NormalTok{(sb, }\DecValTok{0}\NormalTok{, }\DecValTok{0}\NormalTok{, }\DecValTok{10}\NormalTok{);}
  \KeywordTok{store}\NormalTok{.}\FunctionTok{push}\NormalTok{(sb);}
\NormalTok{\});}
\end{Highlighting}
\end{Shaded}
