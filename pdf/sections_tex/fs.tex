...
Synchronous rename(2). Returns \texttt{undefined}.
...
Synchronous fchmod(2). Returns \texttt{undefined}.
...
The synchronous version of \texttt{fs.appendFile}. Returns
\texttt{undefined}.
...
\texttt{fs.exists()} will be deprecated.
...
\begin{Shaded}
\begin{Highlighting}[]
\OtherTok{fs}\NormalTok{.}\FunctionTok{access}\NormalTok{(}\StringTok{'/etc/passwd'}\NormalTok{, }\OtherTok{fs}\NormalTok{.}\FunctionTok{R_OK} \NormalTok{| }\OtherTok{fs}\NormalTok{.}\FunctionTok{W_OK}\NormalTok{, }\KeywordTok{function}\NormalTok{(err) \{}
  \OtherTok{util}\NormalTok{.}\FunctionTok{debug}\NormalTok{(err ? }\StringTok{'no access!'} \NormalTok{: }\StringTok{'can read/write'}\NormalTok{);}
\NormalTok{\});}
\end{Highlighting}
\end{Shaded}

\subsection{fs.accessSync(path{[},
...
\NormalTok{\{ }\DataTypeTok{flags}\NormalTok{: }\StringTok{'w'}\NormalTok{,}
  \DataTypeTok{defaultEncoding}\NormalTok{: }\StringTok{'utf8'}\NormalTok{,}
  \DataTypeTok{fd}\NormalTok{: }\KeywordTok{null}\NormalTok{,}
  \DataTypeTok{mode}\NormalTok{: }\DecValTok{0666} \NormalTok{\}}
\end{Highlighting}
\end{Shaded}
